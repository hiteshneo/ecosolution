...
trait UserAttributes
{
    ...
    public function getChangePasswordButtonAttribute($class)
    {
        if (access()->user()->isAdmin() || (access()->user()->id == $this->id)) {
            return '<a class="'.$class.'" title="Change Password" href="'.route('admin.auth.user.change-password', $this).'">
     ...
     * @return string
     */
    public function getMembershipButtonAttribute($class)
    {
        $countMembership = DB::table('memberships')->where('user_id', $this->id)->where('expired_date','>', date('Y-m-d H:i:s'))->count();
        if($countMembership == 0){
            if (access()->user()->isAdmin() || (access()->user()->id == $this->id)) {
                return '<a class="'.$class.'" title="Membership '.$this->id.'" href="'.route('admin.auth.user.get-membership', $this).'">
                            <i class="fa fa-dollar" data-toggle="tooltip" data-placement="top" title="Membership">
                            </i>
                        </a>';
            }
        }
    ...
    public function getActionButtonsAttribute()
    {
        ...
        if (access()->user()->roles[0]->all) {
            return '<div class="btn-group" role="group" aria-label="'.trans('labels.backend.access.users.user_actions').'">
                    ...
                    '.$this->checkAdmin().'   
                    '.$this->getMembershipButtonAttribute('btn btn-success btn-sm').' 
                </div>';
        }
