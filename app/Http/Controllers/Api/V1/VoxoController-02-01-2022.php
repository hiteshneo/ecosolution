...
class VoxoController extends APIController
{
                ...
                try {
                    ...
                    if ($video_data['status'] == 200) {
                        $output = array(
                            ...
                            "video_id" => $video_data['body']['uri']
                        );
                    }
                    $videoIDArray = explode('/',$video_data['body']['uri']);
                    $videoDetail = json_decode($this->getVideoDetail($videoIDArray[2]));
                    print_r($videoDetail->files[0]->link);die;
                    $output = array(
                        "type" => "success",
                        "link" => $videoDetail['files'][0]['link'],
                        "image_link" => $videoDetail['pictures']['base_link']
                    );
    ...
    function getVideoDetail($videoID)
    {
        ...
        $curl = curl_init();
        $url = "https://v1.nocodeapi.com/voxo/vimeo/krljjYVhdZFpAdZd/videoInfo?video_id=$videoID";
